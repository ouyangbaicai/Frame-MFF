...
j = 0
network_name = 'T/'
DATAPATH = 'RunTimeData/2024-12-07 17.49.48'
    ...
    def FusionProcess(self, model, eval_list_A, eval_list_B, savepath, threshold, network_name):
        ...
        with torch.no_grad():
            for A, B in eval_loader_tqdm:
                ...
                # model2
                # D = einsum('c w h -> w h c', D[0]).clone().detach().cpu().numpy()
                # cv2.imwrite('./Results' + savepath + '/' + self.DATASET_NAME + '-' + str(cnt).zfill(2) + '-dm2.png', D*255)
                # cv2.imwrite('./Results' + savepath + '/' + self.DATASET_NAME + '-' + str(cnt).zfill(2) + '-dm2.png', einsum('c w h -> w h c', D[1]).clone().detach().cpu().numpy()*255)
                # cv2.imwrite('./Results' + savepath + '/' + self.DATASET_NAME + '-' + str(cnt).zfill(2) + '-dm3.png', einsum('c w h -> w h c', D[2]).clone().detach().cpu().numpy()*255)
                # cv2.imwrite('./Results' + savepath + '/' + self.DATASET_NAME + '-' + str(cnt).zfill(2) + '-dm4.png', einsum('c w h -> w h c', D[3]).clone().detach().cpu().numpy()*255)
                cnt += 1
